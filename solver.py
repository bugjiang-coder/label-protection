        ...
        for i in range(NUM_CANDIDATE):
            if i % 3 == ordering[0]:
                if lam21_init:
                    ...
                    lam21 = random.random() * P / p + 1e-32 / d + 1e-32
                lam10, lam11 = None, None
                # print('lam21', lam21)
            elif i % 3 == ordering[1]:
                if lam11_init:
                ...
                if lam10_init:
                    ...
                    lam10 = random.random() * P / (1-p) + 1e-32
...
def solve_small_pos(u, v, d, g, p, P, lam10=None, lam11=None, lam21=None):
    ...
    while True:
        if i % 3 == ordering[0]: # fix lam21
            ...
            # print("C, D, p, g\n", C, D, p, g)
            # print((C + (1 - p) * g) / (C + p * g))
            # yjr 这里我增加了绝对值 ？ 不知道是否正确
            E = math.sqrt(abs((C + (1 - p) * g) / (C + p * g)))
            tau = max((D / p + v - E * u) / (E + (1 - p)/p), 0.0)
            # print('tau', tau)
